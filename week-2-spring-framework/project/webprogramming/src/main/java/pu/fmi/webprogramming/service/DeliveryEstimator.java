...
public class DeliveryEstimator {
  ...
  public LocalDateTime estimateArrivalTime(Delivery delivery) {
    ...
    boolean sameCity = delivery.getWarehouse().getCity()
        .equals(delivery.getCustomer().getCity());

    if (sameCity) {
      estimatedArrival = delivery.getCreatedAt().plusDays(1);
    } else {
      estimatedArrival = delivery.getCreatedAt().plusDays(3);
    }

    if (delivery.getCourier() == null) {
      estimatedArrival = estimatedArrival.plusDays(2);
    }
